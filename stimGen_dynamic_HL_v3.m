function [td,s,frequencyBuffer,isH] = stimGen_dynamic_HL_v3(loFreq,hiFreq,toneDur,toneSOA,trialDur,freqType,cohLevel,changeTime,changeRamp,fs);
% stimGen_static_HL_v3 generates a sequence of tone bursts of 2 frequencies (loFreq & hiFreq)
...
% H for High | L for Low | N for Noise | S for Silence - shorter tones in N
...
% trialDur = 2000; %ms
% freqType = 'HNH';
% cohLevel = [0.65 0.5 0.65];
% changeTime = [500 1000]; %ms
% changeRamp = 1; %ms
...
numNoiseBins = sum(coh == 0.5);
bufferLen = numTonePresentations + numNoiseBins;
frequencyBuffer = ones(bufferLen,1)*hiFreq;
timeBuffer = repmat([toneDur,toneSOA],bufferLen,1);

%% change high freq to low freq depending on coherence level by permuting tones in a block of the same coherence

cohChanges = [1 find(diff(coh))+1 bufferLen+1];
ind = 0;
for cc = 2:length(cohChanges)
    ...
    if ~isnan(coh(ind_coh))
        if coh(ind_coh) == 0.5
            numToneinBloc = numToneinBloc*2;
            numToShuffle = round((1-coh(ind_coh))*numToneinBloc + ((rand(1)-0.5)*0.1));  % add some fluctuations to get low & high trials in 0-coherence trials
            timeBuffer(ind+1:ind+numToneinBloc,1) = toneDur/2;
            timeBuffer(ind+1:ind+numToneinBloc,2) = toneSOA/2;
        else
        ...
        if ~isempty(stateIdx)
            frequencyBuffer(ind+stateIdx) = loFreq; %low
        end
    else
...
frequencyBuffer(frequencyBuffer == 0) = hiFreq;
numLo = sum(frequencyBuffer(:,1) == loFreq);
numHi = sum(frequencyBuffer(:,1) == hiFreq);
...
for ii = 1:bufferLen
    if ~isnan(frequencyBuffer(ii))
        tmax = timeBuffer(ii,1)/1000;                               % Time Duration Of Signal (sec)
    ...
    ind = ind+length(tone)+(timeBuffer(ii,2)/1000*fs);
end
