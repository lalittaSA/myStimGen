function [td,s,frequencyBuffer,isH] = stimGen_noise_embedded_HL_longTest(loFreq,hiFreq,freqType,toneDur,toneIBI,toneAmpL,toneAmpH,lastToneAmp,lastToneDur,fs);
...
% lastToneDur  - duration of the last (decesion) tone
...
trialDur = ((toneDur + toneIBI)*(nTones-1)) + lastToneDur;
...
for ii = 1:nTones
    ...
    if ii == nTones % if last tone
        tmax = lastToneDur/1000;                               % Time Duration Of Signal (sec)
